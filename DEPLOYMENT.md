...
3. Environment variables:
   ...
   | `GEMINI_API_KEY` | alternative to `ANTHROPIC_API_KEY`, from [aistudio.google.com](https://aistudio.google.com) (Anthropic wins if both are set); `GEMINI_MODEL` overrides the chat model (default `gemini-3.5-flash`) and `GEMINI_LITE_MODEL` the cheap tier used for guest chat and suggestions (default `gemini-3.5-flash-lite`) |
   | `ELEVENLABS_API_KEY` | optional, from [elevenlabs.io](https://elevenlabs.io) — high-quality voice-mode TTS (browser speechSynthesis is the fallback); `ELEVENLABS_VOICE_ID` overrides the default British female voice |
